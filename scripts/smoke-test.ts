...
const baseInputs = { ...defaultInputs, useRealValues: false, inflateWithdrawalToRetirement: false };
const boostedInputs = { ...defaultInputs, useRealValues: false, inflateWithdrawalToRetirement: true };
...
const expected = defaultInputs.desiredMonthlyWithdrawal * 12 * Math.pow(1 + defaultInputs.inflationRate / 100, accYears);
...
const result = runMonteCarloSimulation(defaultClient, defaultInputs, defaultPortfolio, settings);
...
const traceWorst = runDetailedSingleSimulation(defaultClient, defaultInputs, defaultPortfolio, settings, result.worstSimIndex!);
const traceBest = runDetailedSingleSimulation(defaultClient, defaultInputs, defaultPortfolio, settings, result.bestSimIndex!);
...
const r1 = runMonteCarloSimulation(defaultClient, defaultInputs, defaultPortfolio, settings);
const r2 = runMonteCarloSimulation(defaultClient, defaultInputs, defaultPortfolio, settings);
const sameKpis =
